...
- **表示層**(`src/`): 分類ロジック(live/today/upcoming、JST 日付グループ化)は `src/lib/classify.ts` の純関数に分離。サムネ URL 解決は `src/lib/thumbnail.ts`(絶対 URL はそのまま、相対パスに BASE_URL 前置)。i18n は `src/lib/i18n.ts` の ja / en 辞書で、`src/pages/index.astro`(ja)と `src/pages/en/index.astro`(en)が `HomePage.astro` に locale を渡す2ページ構成
- **コミュニティ枠**: `data/sponsors.yaml` の PR カードをビルド時に `src/lib/sponsors.ts` が検証・読込(設定エラーはビルド失敗、`until` で掲載期限)。支援導線 URL は `src/lib/site.ts` の `supportUrl`(空文字の間は非表示)
- **データモデル**: `scripts/models.ts` の `Streamer` / `Stream`(status: upcoming | live | ended、platform: youtube | twitch | niconico、欠落は YouTube)。Twitch・ニコニコは追跡集合には入れず表示用 `streams` のみ。YouTube の ended は取得後24時間保持・画面非表示
...
- UI: 日本語主体(`/en/` に英語ページあり。文言変更は `i18n.ts` の両ロケールを更新)・ライトテーマ(明るく軽く。ダークテーマは不採用)・アクセントは赤系のみ(コントラスト規則: 白抜き文字の地色と小さい赤文字は `--accent-deep`、`--accent` は大テキスト・装飾専用)・レスポンシブ・Zen Maru Gothic
